...
public class S2 {
    ...
    public static void main(String[] args) {
        ...
        int width = s.nextInt();
        int[][] m = s.nextIntMatrix(width, height);

        int maxValue = width * height;
        ArrayList<ArrayList<IntPair>> map = new ArrayList<>(maxValue + 1);
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                int value = m[x][y];
                if (value <= maxValue) {
                    ArrayList<IntPair> list = map.get(value);
                    if (list == null) {
                        list = new ArrayList<>();
        ...
        queue.add(new IntPair(width - 1, height - 1));
        while (!queue.isEmpty()) {
            IntPair pos = queue.poll();
            int targetValue = (pos.x + 1) * (pos.y + 1);

            ArrayList<IntPair> targets = map.get(targetValue);
            if (targets != null) {
                for (IntPair target : targets) {
