...
public class S2 {
    ...
    public static void main(String[] args) {
        ...
        int jerseyCount = s.nextInt();
        int personCount = s.nextInt();

        int[] jerseys = new int[jerseyCount]; // 0:none
        for (int i = 0; i < jerseyCount; i++) {
            jerseys[i] = s.nextCharacter();
        }

        int count = 0;
        for (int i = 0; i < personCount; i++) {
            int size = s.nextCharacter();
            int number = s.nextInt() - 1;
            int currSize = jerseys[number];
            if (currSize == 'S' && size == 'S' || (currSize == 'M' && (size == 'S' || size == 'M')) || currSize == 'L') {
                count++;
                jerseys[number] = 0;
            }
        }
