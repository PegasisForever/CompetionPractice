...
public class S3 {
    ...
    public static void main(String[] args) {
        ...
        for (int testI = 0; testI < testCount; testI++) {
            ...
            while (!top.isEmpty() || !branch.empty()) {
                int targetCarNum = lastCarNum + 1;
                if (!branch.empty() && branch.peek() == targetCarNum) {
                    branch.pop();
                    lastCarNum = targetCarNum;
                } else if (!top.isEmpty()) {
                    boolean succeeded = false;
                    while (!top.isEmpty()) {
                        if (top.peek() == targetCarNum) {
                            top.pop();
                            lastCarNum = targetCarNum;
                            succeeded = true;
                            break;
