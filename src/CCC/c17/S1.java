...
public class S1 {
    ...
    public static void main(String[] args) {
        ...
        int count = s.nextInt();
        int[] a = s.nextIntArray(count);
        int[] b = s.nextIntArray(count);

        int[] runningSumA = new int[count];
        int[] runningSumB = new int[count];
        runningSumA[0] = a[0];
        runningSumB[0] = b[0];

        for (int i = 1; i < count; i++) {
            runningSumA[i] = runningSumA[i - 1] + a[i];
            runningSumB[i] = runningSumB[i - 1] + b[i];
        }

        for (int i = count - 1; i >= 0; i--) {
            if (runningSumA[i] == runningSumB[i]) {
