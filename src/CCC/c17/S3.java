...
public class S3 {
    ...
    public static void main(String[] args) {
        ...
        int count = s.nextInt();
        int[] heights = s.nextIntArray(count);

        int[] heightMap = new int[2001];
        ...
        int maxLength = 0;
        int heightCount = 0;
        for (int targetHeight = 2; targetHeight <= 2001; targetHeight++) {
            int startPtr = 1;
            int endPtr = targetHeight - 1;
            int length = 0;
            while (startPtr < endPtr) {
                length += Math.min(heightMap[startPtr], heightMap[endPtr]);
                startPtr++;
                endPtr--;
            }
            if (startPtr == endPtr) {
                length += heightMap[startPtr] / 2;
            }
            if (length == maxLength) {
                heightCount++;
        ...
        for (int targetHeight = 2002; targetHeight <= 4000; targetHeight++) {
            int startPtr = targetHeight - 2000;
            int endPtr = 2000;
            int length = 0;
            while (startPtr < endPtr) {
                length += Math.min(heightMap[startPtr], heightMap[endPtr]);
