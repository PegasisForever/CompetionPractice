...
public class S3B {
    ...
    static boolean[] isPho = new boolean[100001];
    static int[] depth = new int[100001];
    static ArrayList<ArrayList<Integer>> adjOne;
    static int longestPath = -1;

    static void getLongestPath(int current, int last, int currDepth) {
        depth[current] = currDepth;
        longestPath = Math.max(longestPath, depth[current]);
        for (int i = 0; i < adjOne.get(current).size(); i++) {
            if (adjOne.get(current).get(i) == last) continue;
            getLongestPath(adjOne.get(current).get(i), current, currDepth + 1);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int treeWeight = 0;
        int pho = 0;
        adjOne = new ArrayList<>(N + 1);
        for (int i = 0; i < N + 1; i++) {
            adjOne.add(new ArrayList<>());
        }
        ...
        for (int i = 0; i < N; i++) {
            int current = i;
            while (adjOne.get(current).size() == 1) {
                if (isPho[current]) break;
                int node = adjOne.get(current).get(0);
                adjOne.get(current).clear();
                adjOne.get(node).remove(Integer.valueOf(current));
                current = node;
            }
        }

        for (int i = 0; i < N; i++) {
            treeWeight += adjOne.get(i).size();
        }
        getLongestPath(pho, -1, 0);
        for (int j = 0; j < N; j++) {
            if (depth[j] == longestPath) {
                getLongestPath(j, -1, 0);
                break;
            }
        }
