...
public class S3 {
    ...
    public static void main(String[] args) {
        ...
        if (fireCount >= houseCount) {
            System.out.println(0);
            return;
        }

        // house index to space to next house
        IntPair[] spaces = new IntPair[houseCount];
        for (int i = 0; i < houseCount; i++) {
            if (i != houseCount - 1) {
                spaces[i] = new IntPair(i, houses[i + 1] - houses[i]);
            } else {
                spaces[i] = new IntPair(i, 1000000 - houses[i] + houses[0]);
            }
        }
        Arrays.sort(spaces, (a, b) -> b.y - a.y);

        spaces = Arrays.copyOf(spaces, fireCount);
        Arrays.sort(spaces, Comparator.comparingInt(a -> a.x));

        // cut startPos to cut endPos
        IntPair[] cuts = new IntPair[fireCount];
        for (int i = 0; i < fireCount; i++) {
            IntPair space = spaces[i];
            cuts[i] = new IntPair(houses[space.x], houses[space.x] + space.y);
        }
        ...
        for (int i = 0; i < fireCount; i++) {
            ...
            int groupLength;
            if (i == 0) {
                lastCut = cuts[fireCount - 1];
                groupLength = 1000000 - lastCut.y + cut.x;
            } else {
                lastCut = cuts[i - 1];
                groupLength = cut.x - lastCut.y;
            }

            int distance = groupLength / 2;
            if (groupLength % 2 != 0) distance++;
