...
public class S3 {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<IntPair> readings = new ArrayList<>();
        for (int i = 0; i < 1001; i++) {
            int count = counts[i];
            if (count > 0) {
                // Acidic to count
                readings.add(new IntPair(i, count));
            }
        }
        ...
        int firstCount = -1;
        ArrayList<IntPair> firsts = new ArrayList<>();
        int secondCount = -1;
        ArrayList<IntPair> seconds = new ArrayList<>();
        for (IntPair reading : readings) {
            if (firstCount == -1) {
                firsts.add(reading);
                firstCount = reading.y;
            } else if (firstCount == reading.y) {
                firsts.add(reading);
            } else if (secondCount == -1) {
                seconds.add(reading);
                secondCount = reading.y;
            } else if (secondCount == reading.y) {
                seconds.add(reading);
            } else {
                break;
            }
        ...
        if (firsts.size() > 1) {
            System.out.println(Math.abs(firsts.get(0).x - firsts.get(firsts.size() - 1).x));
        } else {
            System.out.println(Math.max(Math.abs(firsts.get(0).x - seconds.get(seconds.size() - 1).x), Math.abs(firsts.get(0).x - seconds.get(0).x)));
        }
    }
}
