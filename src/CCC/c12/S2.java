...
public class S2 {
    ...
    public static void main(String[] args) {
        ...
        IntPair[] pairs = new IntPair[str.length / 2];
        for (int i = 0; i < str.length / 2; i++) {
            IntPair pair = new IntPair(Integer.parseInt(String.valueOf(str[2 * i])), romanToInt(str[2 * i + 1]));
            if (i > 0) {
                IntPair lastPair = pairs[i - 1];
                if (pair.y > lastPair.y) lastPair.x = -lastPair.x;
            }
            pairs[i] = pair;
        }
